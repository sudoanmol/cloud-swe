    ...
    return admission(async (tx) => {
      ...
      if (targetThreadId) {
        const owned = await tx
          .select({ id: thread.id })
          .from(thread)
          .where(and(eq(thread.id, targetThreadId), eq(thread.userId, input.userId)))
          .for("update")
          .limit(1);
  ...
  return {
    ...
    async resetWorkspace({
      ...
      return db.transaction(async (tx) => {
        const current = await lockThreadAndWorkspace(tx, threadId);
        const now = new Date();
        const oldGeneration = current?.generation ?? 0;
        if (oldGeneration !== expectedGeneration) {
          ...
          if (!current)
            throw new ThreadStoreError(
              "RESET_STATE_UNKNOWN",
              "The workspace generation advanced but its workspace row is missing",
              500,
            );
        ...
        let unsettledOlderOperations = 0;
        if (current) {
          ...
          unsettledOlderOperations = olderOperations.length;
    ...
    async cleanupWorkspace({ threadId, transitionId: requestedTransitionId, targetState, mutate }) {
      ...
      return db.transaction(async (tx): Promise<CleanupResult> => {
        ...
        // Keep both row locks while repeating the check immediately before the provider call.
        const rechecked = await cleanupBlockReason(tx, current.id, threadId, current.generation);
        if (rechecked)
          return { outcome: "deferred", reason: rechecked, transitionId, workspace: current };
